...
Perks are auto-detected from character items. Some have built-in system AEs (handled natively by Vagabond), some are automated by VCE via runtime hooks, and the rest are tracked for future automation. This page is the single source of truth for all 104 perks.
...
The 15 perks below have full or partial implementation. The remaining 89 are listed in the [Full Perk List](#full-perk-list-alphabetical) below.
...
| Protector | ✅ Module | Auto-rolls Endure save when ally fails save near attacker; heals for highest die on pass |
| Briar Healer | ✅ Module | While caster Focuses on Life: target gains +1 Armor + reactive d6 thorn damage to any Being who melees them. Subscribes to `vagabond.postDamageApply` (v5.3.0+). |
