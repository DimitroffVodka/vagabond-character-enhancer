...
Spells in the Vagabond system have no built-in Active Effects. VCE automates select spells via runtime hooks, chat-card buttons, an effect-only handler for any spell with `causedStatuses` configured, and the unified [Companion System](companions.md) for spells that summon creatures. This page is the single source of truth for all 59 spells.
